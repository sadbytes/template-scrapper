...
def fetch_category_templates(category_id, category_name, force=False):
        ...
        if not force:
            ...
            try:
                with open(f'{CURRENT_DIR}/wix/cache/{category_id}.json', 'r') as f:
                    ...
                    logger.info(f'Loaded cache of {category_name} from {category_id}.json.')
                    return data
            except:
                logger.exception(f'An error has occured while reading cache: {CURRENT_DIR}.json') 
        logger.info(f'Performed forced data request and cleared old cache for {category_name}. ({category_id})')   
    ...
    if response.status_code == 200:
        ...
        logger.error(f"Failed to get data for {category_name}. Status code: {response.status_code} ({url})")
        return None
...
def cache_api_data(data, category_name, category_id):
    ...
    try:
        ...
        logger.exception(f"Error caching data for {category_name} ({category_id}).")
...
def main(logging=True):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=MAX_THREADS) as executor:
        for category in categories:
            tasks[executor.submit(fetch_category_templates, *[category['categoryId'], category['displayName']])] = category
            for sub in category['subCategories']:
                tasks[executor.submit(fetch_category_templates, *[sub['categoryId'], sub['displayName']])] = sub

        for future in concurrent.futures.as_completed(tasks):
            c = tasks[future]
            logger.info(f'Completed retrieving data for {c['displayName']} ({c['categoryId']}).')
